...
他会羞耻得浑身颤抖。
顺着腰摸到他白花花的----，说来也奇怪，明明哪里都瘦，偏偏屁股肉感十足。
一巴掌下去，汤浩抑制不住地呻吟出声，单薄的肩胛骨一耸一耸的，像是要突破薄薄的皮肤，长出一双翅膀。
即使是-功能障碍，看到此时汤浩的脸也会勃起吧。
但是此时此刻的汤浩，只属于我一个人。
...
或许我遇见的人从来都不是汤浩，他一直都应该是伯远才对。`
  ...
  {
    title: '《杨老师》',
    preview: ['今儿是杨戬第一天上班。', '大学生逃不过选修课，听又听不进，不去又怕点名，反正就是烦的不行。', '杨戬踩着点走进了课室，手中还拿着个冒着热气的保温杯。'],
    fullText: `今儿是杨戬第一天上班。
...
function CollectionMosaic({
  ...
  if (group.layout === 'vertical-pair') {
    return (
      <div className={`grid gap-2.5 md:gap-3 ${className}`}>
      ...
      <main className="mx-auto max-w-7xl px-4 pb-16 pt-4 sm:px-5 md:px-8 md:pb-20 md:pt-6 lg:px-10">
        <header className="sticky top-0 z-30 -mx-4 mb-8 border-b border-paper-200/60 bg-[rgba(251,248,245,0.82)] px-4 py-3 backdrop-blur sm:-mx-5 sm:px-5 md:-mx-8 md:mb-10 md:px-8 md:py-4 lg:-mx-10 lg:px-10">
          <div className="flex flex-col gap-3 text-[11px] uppercase tracking-[0.22em] text-paper-500 md:flex-row md:items-center md:justify-between md:gap-4 md:text-xs md:tracking-[0.26em]">
            <span className="shrink-0">曹雯一 - 作品集</span>
          ...
          <div className="space-y-16 md:space-y-24">
            ...
            <div>
              <div className="mb-6">
                ...
                <div className="mt-5 inline-block rounded-[1.2rem] bg-gradient-to-br from-paper-100/80 to-paper-50/60 px-5 py-3.5 ring-1 ring-paper-200/70 md:mt-6 md:rounded-[1.3rem] md:px-6 md:py-4">
                  <p className="mb-3 text-[10px] font-medium uppercase tracking-[0.26em] text-paper-600 md:text-[11px]">物料类型</p>
                  <div className="flex flex-wrap gap-x-3 gap-y-2">
                    {['吧唧', '透卡', '透扇', '异形贴纸', '票根', '口罩', '纸袋', '书签', '手幅', '邮票', '手环'].map((item) => (
                      <span key={item} className="rounded-lg bg-white/70 px-4 py-1.5 text-base text-paper-700 ring-1 ring-paper-200/50">{item}</span>
                    ))}
                  </div>
                </div>
              </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {activityMerchImages.map((image, index) => (
                  <article key={image} className="overflow-hidden rounded-[1.4rem] bg-white/70 shadow-soft ring-1 ring-paper-200/70 md:rounded-[1.6rem]">
                    <img src={image} alt={`《新神榜：杨戬》线下活动物料 ${index + 1}`} className="w-full transition duration-500 hover:scale-[1.02]" />
                  </article>
                ))}
              </div>
            </div>
        ...
        <section id="content" className="scroll-mt-28 py-12 md:scroll-mt-32 md:py-14">
          <SectionHeading eyebrow="02 · Content Creation" title="同人内容产出" />
        ...
        <section id="collection" className="scroll-mt-28 py-12 md:scroll-mt-32 md:py-14">
          ...
          <div className="mb-8 md:mb-10">
            <div className="inline-block rounded-[1.2rem] bg-gradient-to-br from-paper-100/80 to-paper-50/60 px-5 py-3.5 ring-1 ring-paper-200/70 md:rounded-[1.3rem] md:px-6 md:py-4">
              <p className="mb-2 text-[10px] font-medium uppercase tracking-[0.26em] text-paper-600 md:text-[11px]">吃谷 IP</p>
              <div className="flex flex-wrap gap-x-3 gap-y-2">
                {['恋与深空', '恋与制作人', 'Chiikawa', '线条小狗', '三丽鸥', '未来都市No.6', 'NANA', '其他二次元IP'].map((item) => (
                  <span key={item} className="rounded-lg bg-white/70 px-4 py-1.5 text-base text-paper-700 ring-1 ring-paper-200/50">{item}</span>
                ))}
              </div>
            </div>
